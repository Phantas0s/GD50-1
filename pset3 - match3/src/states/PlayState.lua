            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    if self:calculateMatches() then
                        -- if match is found highlightedTile is set to nil
                        self.highlightedTile = nil
                    else
                        -- swap back grid positions of tiles
                        ...
                        self.wasHighlighted = self.highlightedTile
                        self.highlightedTile = nil

                        Timer.after(0.1, function()
                            Timer.tween(0.1, {
                                [self.wasHighlighted] = {x = newTile.x, y = newTile.y},
                                [newTile] = {x = self.wasHighlighted.x, y = self.wasHighlighted.y}
                            })
                        end)
                    end
                end)
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for k, tile in pairs(match) do
                -- the highest the variety the highest the reward, normal tile give no additional point
                bonus = bonus + 10 * (tile.variety - 1)
            end
        ...
        Timer.tween(0.25, tilesToFall):finish(function()
            ...
            if not self:calculateMatches() then
                -- if new board has no possible matches initiate a new one
