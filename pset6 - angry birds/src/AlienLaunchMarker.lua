...
function AlienLaunchMarker:init(world)
    ...
    -- our first alien we will eventually spawn
    self.alien = nil

    -- stores all the aliens in play
    self.aliens = {}
...
function AlienLaunchMarker:update(dt)
    ...
    elseif love.keyboard.wasPressed('space') and not self.split and not self.alien.collided then
        self.split = true
        ...
        for i = -1, 1, 2 do
            -- generates a new alien at the same coordinates of the original one
            ...
            -- change linear velocity to make the aliens split
            alien.body:setLinearVelocity(velocityX, velocityY + 100 * i)
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        -- render all the aliens in our table
        for k, alien in pairs(self.aliens) do
            alien:render()
        end
    end
