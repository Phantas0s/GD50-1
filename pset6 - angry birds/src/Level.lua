...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        -- if the player collided with something it is not able to split anymore
        if types['Player'] then

            -- flag the alien at collision
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        -- iterate through the aliens in the table and check wheter they are in game or not
        for k, alien in pairs(self.launchMarker.aliens) do
            ...
            -- if any alien is still moving withing the boundaries of the screen none is destroyed
            if (xPos > 0 and xPos < VIRTUAL_WIDTH) and (math.abs(xVel) + math.abs(yVel) > 1.5) then
                goto continue
            end
        end
        -- else all of them are destroyed and a new launchMarker is initialized
        ...
        -- skip the reset of the launchMarker
        ::continue::
    end
end
