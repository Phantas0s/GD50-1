...
public class LevelGenerator : MonoBehaviour {
	...
	// allows us to spawn holes in the floor
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				// spawn holes with a 1 in 200 chance but not directly below the character
				if (generateHoles) {
					if (characterController.transform.position == new Vector3(x, 1, z) || ((int) Random.Range(1, 200) != 1)) {
						// create floor
